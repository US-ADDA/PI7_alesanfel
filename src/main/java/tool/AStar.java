...
public class AStar<P extends Problem, S extends Solution, H extends Heuristic<P>> {
    ...
    public List<Integer> search() {
        P vertexActual = null; // Vértice actual.
        while (!heap.isEmpty() && Boolean.FALSE.equals(goal)) { // Mientras no se vacíe el heap y no se haya llegado al último vértice.
            Handle<Double, SubAStar<P>> handle = heap.deleteMin(); // Elimina el mínimo de la cola.
            SubAStar<P> dataActual = handle.getValue(); // Obtiene el valor del nodo.
            vertexActual = dataActual.vertex(); // Obtiene el vértice del nodo.
            for (Integer action : vertexActual.actions()) { // Recorre las acciones del vértice.
                P neighbor = vertexActual.neighbor(action); // Obtiene el vecino de la acción.
                Double newDistance = dataActual.distanceToOrigin() - vertexActual.weight(action); // Calcula la nueva distancia, (siempre es negativa).
                ...
                if (!tree.containsKey(neighbor)) { // Si no está en el árbol.
                    SubAStar<P> ac = SubAStar.of(neighbor, action, vertexActual, newDistance); // Crea el nodo.
                    ...
                    heapVertex.decreaseKey(newDistanceToEnd); // Cambia la distancia del nodo.
                }
            }
            goal = vertexActual.goal().test(vertexActual); // Comprueba si es hemos terminado.
        }
        return actions(vertexActual); // Devuelve las acciones.
    }
    ...
    public record SubAStar<P extends Problem>(P vertex, Integer action, P lastVertex, Double distanceToOrigin) {
        public static <P extends Problem> SubAStar<P> of(P vertex, Integer a, P lastVertex,
                                                         Double distanceToOrigin) {
            return new SubAStar<>(vertex, a, lastVertex, distanceToOrigin);
        }
    }
}
